...
class categoryTitle(View):
    def get(self, request, val):
        ...
        title = Product.objects.filter(category=product[0].category).values('title')  # noqa
        return render(request, "shop/category.html", locals())
...
class ProfileView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            reg = Customer(user=user, name=name, locality=locality, city=city, mobile=mobile, county=county, postcode=postcode)  # noqa
            reg.save()
            messages.success(request, "congradulations!! profile save successfully")  # noqa
        else:
...
class updateAddress(View):
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            ...
            add.save()
            messages.success(request, "congratulations! profile update successfully")  # noqa
        else:
            messages.warning(request, "Invalid input data")
...
def search(request):
    ...
    product = Product.objects.filter(Q(title__icontains=query) | Q(category__icontains=query))  # noqa
